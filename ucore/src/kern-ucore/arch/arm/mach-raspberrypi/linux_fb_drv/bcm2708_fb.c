...
const char *bcm2708_name = "BCM2708 FB";

#define DRIVER_NAME "bcm2708_fb"

u32 dma_busy_wait_threshold = 1<<15;
module_param(dma_busy_wait_threshold, int, 0644);
MODULE_PARM_DESC(dma_busy_wait_threshold, "Busy-wait for DMA completion below this area");

int fbswap = 0;     /* module parameter */
...
void bcm2708_fb_debugfs_deinit(struct bcm2708_fb *fb)
{
	debugfs_remove_recursive(fb->debugfs_dir);
	fb->debugfs_dir = NULL;
}

int bcm2708_fb_debugfs_init(struct bcm2708_fb *fb)
{
...
int bcm2708_fb_set_bitfields(struct fb_var_screeninfo *var)
{
	int ret = 0;
...
int bcm2708_fb_check_var(struct fb_var_screeninfo *var,
				struct fb_info *info)
{
...
int bcm2708_fb_set_par(struct fb_info *info)
{
	uint32_t val = 0;
...
inline u32 convert_bitfield(int val, struct fb_bitfield *bf)
{
...
int bcm2708_fb_setcolreg(unsigned int regno, unsigned int red,
...
int bcm2708_fb_blank(int blank_mode, struct fb_info *info)
{
	/*print_debug("bcm2708_fb_blank\n"); */
	return -1;
}

void bcm2708_fb_fillrect(struct fb_info *info,
...
void set_dma_cb(struct bcm2708_dma_cb *cb,
		       int        burst_size,
		       dma_addr_t dst,
		       int        dst_stride,
		       dma_addr_t src,
...
void bcm2708_fb_copyarea(struct fb_info *info,
...
void bcm2708_fb_imageblit(struct fb_info *info,
...
irqreturn_t bcm2708_fb_dma_irq(int irq, void *cxt)
{
...
int fbwidth = 800;	/* module parameter */
int fbheight = 480;	/* module parameter */
int fbdepth = 16;	/* module parameter */

int bcm2708_fb_register(struct bcm2708_fb *fb)
{
	int ret;
	dma_addr_t dma;
	void *mem;
...
int bcm2708_fb_remove(struct platform_device *dev)
{
...
struct platform_driver bcm2708_fb_driver = {
...
int __init bcm2708_fb_init(void)
{
...
void __exit bcm2708_fb_exit(void)
{
